...
5. **Rerank + size-sort** — cross-encoder (`models/reranker.py`) rescores the top ~50, then `apply_size_sort` re-tiers by whether the doc's size matches the query's size anchors (the cross-encoder is size-blind). Size sort is a safe no-op when the query has no size token.
