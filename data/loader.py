...
# ---------------------------------------------------------------------------
# Enrichment cache  (generated by scripts/enrich_descriptions.py)
# ---------------------------------------------------------------------------

_CACHE_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "enrichment_cache.json")
_ENRICHMENT_CACHE: dict = {}

if os.path.exists(_CACHE_PATH):
    with open(_CACHE_PATH) as _f:
        _ENRICHMENT_CACHE = json.load(_f)
    print(f"  [INFO] Enrichment cache loaded: {len(_ENRICHMENT_CACHE)} entries")
...
def load_au_parspec() -> list:
    ...
    for _, row in deduped.iterrows():
        ...
        point_id = make_id(source, iid)
        ext_desc = _ENRICHMENT_CACHE.get(point_id)
        bm25_text = f"{desc} {ext_desc}" if ext_desc else desc
    ...
    for _, row in deduped.iterrows():
        ...
        records.append({
            ...
            **rollup,
            ...
            "_bm25_desc":           normalize_specs(f"{bm25_text} {mfr}"),
        })
    return records
...
def load_inventory_sample() -> list:
    ...
    for _, row in df.iterrows():
        ...
        point_id = make_id(source, iid)
        ext_desc = _ENRICHMENT_CACHE.get(point_id) or ext  # cache first, then CSV value
        bm25_text = f"{desc} {ext_desc}" if ext_desc else desc
        ...
        records.append({
            ...
            "locations":            locations,
            "_bm25_model":          model_number_variants(model),
            "_bm25_desc":           normalize_specs(f"{bm25_text} {mfr} Fuses"),
        })
    return records
